...
router.post('/search', (req, res) => {
    ...
    if (capability) {
        agents = agents.filter(a => a.capabilities.some((c) => c.toLowerCase().includes(capability.toLowerCase())));
    }
    res.json(agents);
});
...
router.post('/', (req, res) => {
    ...
    const { name, description, version, code_hash, storage_provider, storage_cid, capabilities, runtime } = req.body;
    if (!name || !code_hash) {
